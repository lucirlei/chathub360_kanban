...
module ChatwootApp
  ...
  def self.ensure_stacklab_service_loaded
    @_stacklab_service_loaded_status ||= begin
      if defined?(::Stacklab::LicensingService)
        ...
        # Este log pode ser redundante se o require_relative no topo já logou, mas serve como uma dupla checagem.
        Rails.logger.error "Stacklab::LicensingService não está definido após a tentativa de require_relative. Funcionalidades PRO desabilitadas."
        false
      end
  ...
  def self.stacklab?
    self.stacklab.kanban_pro_active?
  end
end
