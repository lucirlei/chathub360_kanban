class Api::V1::Accounts::ConversationsController < Api::V1::Accounts::BaseController
  ...
  def search
    result = conversation_finder.perform
    @conversations = result[:conversations].includes(:kanban_items)
    @conversations_count = result[:count]
  end
  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      # Trigger: Criação automática de KanbanItem na etapa de entrada do funil
      Current.account.funnels.active.each do |funnel|
        next unless funnel.stages.present?

        funnel.stages.each do |stage_id, stage|
          next unless stage['is_entry']
  ...
  def conversation
    @conversation ||= Current.account.conversations.includes(:kanban_items).find_by!(display_id: params[:id])
